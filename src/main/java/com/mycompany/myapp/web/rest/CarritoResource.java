...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Carrito}.
 */
@RestController
@RequestMapping("/api")
public class CarritoResource {
     ...
     */
    @GetMapping("/carritos")
    public ResponseEntity<List<CarritoDTO>> getAllCarritos(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Carritos");
        Page<CarritoDTO> page;
        if (eagerload) {
            page = carritoService.findAllWithEagerRelationships(pageable);
        } else {
            page = carritoService.findAll(pageable);
        }
