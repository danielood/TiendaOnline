...
export class ValoracionesComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.valoracionesService
      ...
      .subscribe(
        (res: HttpResponse<IValoraciones[]>) => this.paginateValoraciones(res.body, res.headers),
  ...
  trackId(index: number, item: IValoraciones) {
    return item.id;
  }

  registerChangeInValoraciones() {
    this.eventSubscriber = this.eventManager.subscribe('valoracionesListModification', response => this.loadAll());
  }
  ...
  protected paginateValoraciones(data: IValoraciones[], headers: HttpHeaders) {
